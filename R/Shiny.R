  ...
  sidebarLayout(
    sidebarPanel(
      ...
      selectInput(
        ...
        choices = c("Euro 95", "Diesel", "LPG", "Heating Oil", "Fuel Oil")
        selected = "Euro 95"
      ),
      dateRangeInput(
...
server <- function(input, output) {
  oil_bulletin_long <- fread("https://raw.githubusercontent.com/Vosbrucke/Fuel_prices_in_europe/main/R/oil_bulletin_long.csv")
  ...
  output$fuel_prices <- renderPlot({
    tryCatch(
    {
      ...
      dt_plot <- oil_bulletin_long[countries_letters, on = "country_name"]
      
      # the start of y axis
      y_axis <- dt_plot[1, (value), by = country]
      ...
      if (shinybrowser::get_width() > 1500) {
        title <- paste("Price of", tolower(input$selected_fuel), "among EU Member States on", format(max(react_df()[["date"]]), "%d %B %Y")) 
      } else {
        title <- paste("Price of", tolower(input$selected_fuel), "among\nEU Member States on", format(max(react_df()[["date"]]), "%d %B %Y")) 
      }
      
      ggplot(data = fuel_price_EU_0(), aes(x = date, y = value, group = country_name)) +
